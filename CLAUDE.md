...
Anything that needs a real Secure Enclave key is marked `.enabled(if: SecureEnclave.isAvailable)`, so those tests run on a Mac and skip on a CI runner.
